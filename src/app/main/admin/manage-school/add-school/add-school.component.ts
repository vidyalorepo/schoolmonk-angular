...
export class AddSchoolComponent implements OnInit {
  ...
  submitAddSchoolRecord() {
    // const data = this.addSchoolForm.value;
    //   data.schoolStatus = "In-Active";
    //   data.featuredSchool = (data.featuredSchool == true ? 'Y': 'N');
    //   data.customAdmissionForm = (data.customAdmissionForm == true ? 'Y': 'N');
    ...
    if (this.addSchoolForm.status == 'VALID') {
      ...
      // return
      this.openLoader();
      this._authService.request('post', 'schooluser/register', data).subscribe(
        (res) => {
          console.log(res);
          this.closeLoader();
          if (res.status == 200) {
            this.messageService.clear();
            this.messageService.add({severity:'success', summary:'', detail:'An activation link is sent to the school administrator’s email. Click on the activation like to activate the account'});
            this.router.navigate(['/manage-school/school-list/active&in-active']);
          }
        },
        (error) => {
          console.log(error);
