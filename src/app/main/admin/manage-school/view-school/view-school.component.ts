...
export class ViewSchoolComponent implements OnInit {
  ...
  steps: any[] = [
    ...
    { title: 'Address', value: '2', completed: false, msg: 'Address added.' },
    { title: 'Level', value: '3', completed: false, msg: 'Level added.' },
    { title: 'Timing', value: '4', completed: false, msg: 'Timing added.' },
    {
      title: 'Documents',
      value: '5',
      completed: false,
  ...
  initAddSchoolForm = () => {
    this.addSchoolForm = this.formBuilder.group({
      ...
      schoolPrincipalName: ['', Validators.compose([Validators.required])],
  ...
  getSchoolLevel() {
    ...
    this._authService
      .request('get', 'commonMaster/getSchoolLevels')
      .subscribe((response) => {
  ...
  selectSclLevel(level: any, index: any) {
    ...
    for (let i = 0; i < this._schoolLevelInfoList.length; i++) {
      if (this._schoolLevelInfoList[i].schoolLevelName === level) {
        (
          (this.addSchoolForm.get('schoolLevelDtlVo') as FormArray).at(
            index
          ) as FormGroup
        ).patchValue({
          stream: [''],
        ...
        if (this._schoolLevelInfoList[i].toClass === 'XII') {
          ...
          this.patchStream(
            index,
            (this.addSchoolForm.get('schoolLevelDtlVo') as FormArray).at(index)
              .value
          );
        } else {
  ...
  getFromClassList() {
    this._classFromList = [];
    this._authService
      .request('get', 'commonMaster/getClassFromList')
      .subscribe((response) => {
        if (response.status == 200) {
  ...
  getUptoClassList() {
    this._uptoClassList = [];
    this._authService
      .request('get', 'commonMaster/getClassUptoList')
      .subscribe((response) => {
        if (response.status == 200) {
  ...
  classFromChange(id?: any, cls?: any) {
    (
      (this.addSchoolForm.get('schoolBoardClassDtlVo') as FormArray).at(
        id
      ) as FormGroup
    )
      .get('startClassSlNo')
      .patchValue(cls.split('/')[0]);
    (
      (this.addSchoolForm.get('schoolBoardClassDtlVo') as FormArray).at(
        id
      ) as FormGroup
  ...
  classUpToChange(id?: any, cls?: any) {
    ...
    if (cls) {
      this._authService
        .request(
          'get',
          'commonMaster/getClassStreams?className=' + cls.split('/')[1]
        )
        .subscribe(
          (response) => {
            if (response.status == 200) {
              ...
              (
                (
                  this.addSchoolForm.get('schoolBoardClassDtlVo') as FormArray
                ).at(id) as FormGroup
              )
                .get('tempStream')
                .patchValue(this._classStreamsList);
  ...
  getBoardList() {
    this._boardList = [];
    this._authService
      .openRequest('get', 'commonMaster/getBoards')
      .subscribe((response) => {
        if (response.status == 200) {
  ...
  getMediumList() {
    this._mediumList = [];
    this._authService
      .openRequest('get', 'commonMaster/getMediums')
      .subscribe((response) => {
        if (response.status == 200) {
  ...
  getSchoolType() {
    ...
    this._authService
      .openRequest('get', 'commonMaster/getSchoolTypes')
      .subscribe((response) => {
        if (response.status == 200) {
  ...
  patchForm(details: any) {
    ...
    this.addparameterList(
      details.schoolLevelDtlVo.length,
      details.schoolLevelDtlVo
    );
    this.onState();
  ...
  patchStream(i?: any, value?: any) {
    ...
    if (value.toClass === 'XII') {
      this._authService
        .request(
          'get',
          'commonMaster/getClassStreams?className=' + value.toClass
        )
        .subscribe(
          (response) => {
            console.log(response);
            if (response.status == 200) {
              ...
              (
                ...
                .get('stream')
                .patchValue(value.stream != '' ? value.stream.split(',') : '');
            }
          },
          (error) => {
            this._classStreamsList = [];
  ...
  // Save Update
  submitUpdateSchoolRecord(profileStep: any) {
    this.submitted = true;
    const data = this.addSchoolForm.value;
    this.existingSteps.push(profileStep);
    data.profileStepSet = this.existingSteps;
    data.contactUser = {
      userId: this._schoolsDetails.contactUser
        ? this._schoolsDetails.contactUser.userId
        : null,
    };
    data.schoolClass = data.schoolClass.toString();
    if (data.featuredSchool == true) {
      data.featuredSchool = 'Y';
    } else if (data.featuredSchool == false) {
      data.featuredSchool = 'N';
    }

    if (data.customAdmissionForm == true) {
      data.customAdmissionForm = 'Y';
    } else if (data.customAdmissionForm == false) {
      data.customAdmissionForm = 'N';
    }
    ...
    if (this.addSchoolForm.status == 'VALID') {
      ...
      // return
      this._authService
        .request('post', `schooluser/schoolUpdate`, data)
        .subscribe(
          (res) => {
            console.log(res);
            this.closeLoader();
          ...
          image.onload = (rs: any) => {
            ...
            if (rs.currentTarget['width'] / rs.currentTarget['height'] == 1) {
              ...
              this._authService
                .fileRequest(
                  'post',
                  `fe/uploadSingleFileInFolder?formCode=school_media&txId=${
                    this._schoolID
                  }&docType=${doc}&fileId=${id || ''}`,
                  formData
                )
                .subscribe(
                  (fileRes) => {
                    ...
                    if (fileRes) {
                      if (doc == 'school_logo') {
                        this._schoolLogoUrl = fileRes;
                      } else if (doc == 'school_banner') {
                        this._schoolBannerUrl = fileRes;
                      ...
                      } else if (doc == 'school_Brochure') {
                        this._schoolBrochureUrl = fileRes;
  ...
  uploadSingleFile(event: any, doc: any, id?: any) {
    ...
    this._authService
      .fileRequest(
        ...
        `fe/uploadSingleFileInFolder?formCode=school_media&txId=${
          this._schoolID
        }&docType=${doc}&fileId=${id || ''}&profileStep=${profileStepTemp}`,
        formData
      )
      .subscribe(
        (fileRes) => {
  ...
  uploadMultipleFile(event: any, doc: any) {
    ...
    this._authService
      .fileRequest(
        'post',
        `fe/uploadMultipleFileInFolder?formCode=school_media&txId=${this._schoolID}&docType=${doc}`,
        formData
      )
      .subscribe(
        (fileRes) => {
    ...
    this._authService.request('get', 'schooluser/school/' + id).subscribe(
      (res) => {
        ...
        if (res) {
          ...
          console.log(
            '_schoolLevelInfoListTime :',
            this._schoolLevelInfoListTime
          );
          if (res.schoolTimingDtlVo.length > 0) {
            this.addPatchParameterListTime(
              res.schoolTimingDtlVo.length,
              res.schoolTimingDtlVo
            );
          } else {
    ...
    if (level) {
      for (let i = 0; i < this._schoolLevelInfoListTime.length; i++) {
        if (this._schoolLevelInfoListTime[i].schoolLevelName === level) {
          (
            (this._schoolForm.get('schoolTimingDtlVo') as FormArray).at(
              index
            ) as FormGroup
          ).patchValue({
  ...
  submitTiming() {
    ...
    if (this._schoolForm.status == 'VALID') {
      ...
      this.openLoader();
      this._authService
        .request('post', `schooluser/saveSchoolTimingDtl`, data)
        .subscribe(
          (res) => {
            this.closeLoader();
            console.log(res);
  ...
  deleteInfraSt(id?: any) {
    ...
    this._authService
      .request('post', `schooluser/deleteSchoolInfraDtl`, data)
      .subscribe((res) => {
        console.log(res);
        if (res.status == 200) {
  ...
  updateAcademic() {
    ...
    if (this._academicForm.status == 'VALID') {
      ...
      this.openLoader();
      this._authService
        .request('post', `schooluser/saveSchoolAcademicDtl`, data)
        .subscribe(
          (res) => {
            this.closeLoader();
            console.log(res);
  ...
  deleteAcademic(id?: any) {
    ...
    this.openLoader();
    this._authService
      .request('post', `schooluser/deleteSchoolAcademicDtl`, data)
      .subscribe(
        (res) => {
          this.closeLoader();
          console.log(res);
  ...
  updateContactInfo() {
    ...
    if (data) {
      this.openLoader();
      this._authService.request('post', `users/activateUser`, data).subscribe(
        (res) => {
          this.closeLoader();
          console.log(res);
          if (res.status == 200) {
